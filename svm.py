...
Original file is located at
    https://colab.research.google.com/drive/1bLKAmziP84pjnLmD3SF3BcU08PjGJgIs
"""
...
Hinge loss는 margin이 0인 지점에서 미분이 불가능하다. 애초에 gradient가 정의되지 않는 점이므로 임의로 정하는 수 밖에 없다. 내가 구현한 코드를 보면 margin이 0보다 클 때만 loss에 더해주고 0보다 작거나 같을 때에는 더해주지 않는다. dMargin/dScore를 구한다고 하자. Score = 2, correct_score = 3이라고 하면 margin = 2-3+1 = 0이다. 그러므로 if문 안에 들어가지 못하기 때문에 gradient를 더해주지 않는다(0이다). 이제 Numerical gradient를 구해보자. h=0.001로 가정하면 gradient는 (0.001-0)/0.001 = 1이 된다.
...
$\color{blue}{\textit Your Answer:}$ *fill this in*  
해당 객체가 평균적으로 보이는 모양. score는 샘플이랑 가중치의 내적이라서 정확한 label에서 더 높은 점수를 얻으려면 가중치랑 sample이 평행해야 한다.
