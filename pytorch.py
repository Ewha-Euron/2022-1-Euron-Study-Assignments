...
Original file is located at
    https://colab.research.google.com/drive/1bghVTfA3AGyjVWH1wCbxZY9vpsNHooNY
...
class ThreeLayerConvNet(nn.Module):
    def __init__(self, in_channel, channel_1, channel_2, num_classes):
        ...
        self.conv_w1 = nn.init.kaiming_normal_(self.cv1.weight)
        self.conv_b1 = self.cv1.bias.data.fill_(0.01)

        self.conv_w2 = nn.init.kaiming_normal_(self.cv2.weight)
        self.conv_b2 = self.cv2.bias.data.fill_(0.01)
        
        self.fc_w = nn.init.kaiming_normal_(self.fc.weight)
    ...
    def forward(self, x):
        ...
        conv_b1 = self.conv_b1
        conv_w2 = self.conv_w2
        conv_b2 = self.conv_b2
...
learning_rate = 1e-3
model = ThreeLayerConvNet(3, 16, 16, 10)
...
class ResNet(nn.Module) :
    def __init__(self, Block, n) :
        ...
        self.resnet_start = nn.Sequential(
            nn.Conv2d(3, 16, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(16),
            nn.ReLU()
        )
        self.n = n
        self.stack1 = self.block_stack(Block, 16, 16, 1, False)
        self.stack2 = self.block_stack(Block, 16, 32, 2, True)
        self.stack3 = self.block_stack(Block, 32, 64, 2, True)
        self.resent_end = nn.Linear(64, 10)

    def block_stack(self, block, in_channels, out_channels, stride, downsample):
        layer = []
        for i in range(self.n) :
            if downsample == True and i == 0 :
                layer.append(block(in_channels, out_channels, stride))
            else :
    ...
    def forward(self, x):
        x = self.resnet_start(x)
        x = self.stack1(x)
        x = self.stack2(x)
        x = self.stack3(x)
        x = x.mean((2,3))
        output = self.resent_end(x)
        return output
...
I use ResNet(with residual block) and adma optimizer
